...
# with centered body size and a GLM with identity link
# Used by 'cti_modelEval_cent.R')
...
plotDerivative <- function(fit0, skelagePred=seq(8,18,2), birthdayPred=seq(1930,1995,0.5), alpha=0.01, B=10000, eps=1e-7){
  ...
  return(list(z=z, skelagePred=skelagePred, birthdayPred=birthdayPred,
        outcome="Derivative wrt birth day",
        ylab="d jo/R",
        alpha=alpha))
}

plotPerChange <- function(fit0, initialBirthday=1930, endBirthday=1995, skelagePred=seq(6,18,.1), alpha=0.01, B=10000){
  ...
  return(list(ggAll=ggAll, z=z, zAll=zAll, initialBirthday=initialBirthday,
          endBirthday=endBirthday,
        outcome="percent change wrt birthday"))
}
