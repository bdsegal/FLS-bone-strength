...
paperPath <- file.path(computer,
  "Dropbox/Research/Bones/final_analysis/cti/paper")

setwd(file.path(computer,"Dropbox/Research/Bones/final_analysis/cti"))
...
for (i in 1:length(files)){
  ...
  for (m in 1:M){
    ...
    trendTemp <- plotTrend1(multImpute[[m]], B=B, cti=TRUE,
    ...
    derivTemp <- plotDerivative1(multImpute[[m]], B=B, cti=TRUE,
      skelagePred=skelagePred,
      birthdayPred=birthdayPred)
    ...
    pcTemp <- plotPerChange1(multImpute[[m]], B=B, cti=TRUE,
        skelagePred=skelagePredPc, 
...
ggAll <- ggplot(aes(x=skelage, y=mean), data=zAll)+
  geom_line()+
  ...
  labs(
    ...
    y="Percent change in\ncortical thickness index", x="Skeletal age")+
...
ggAll 
ggsave(file.path(paperPath, "m1ctiCentPhvageMultImputepc.png"))
...
ggsave(file.path(paperPath, "m1ctiCentPhvageMultImputeMaleMean.png"))
...
ggplot(aes(x=birthday, y=mean), data=slice)+
    ...
    theme(axis.text.x = element_text(angle=45, hjust = 1, vjust=1))+
    scale_y_continuous(lim=c(0,0.65))
ggsave(file.path(paperPath, "m1ctiCentPhvageMultImputeFemaleMean.png"))
...
ggsave(file.path(paperPath, "m1ctiCentPhvageMultImputeMaleDer.png"))
...
ggplot(aes(x=birthday, y=mean), data=slice)+
    ...
    theme(axis.text.x = element_text(angle=45, hjust = 1, vjust=1))+
    scale_y_continuous(lim=c(-0.0025, 0.0075))
ggsave(file.path(paperPath, "m1ctiCentPhvageMultImputeFemaleDer.png"))
...
ggsave(file.path(paperPath, "ImputeBetas_cti.png"))
...
ggsave(file.path(paperPath, "ImputeBetasLog_cti.png"))
...
main=paste(names(out$z)[i],": ", (1-out$alpha)*100,"% credible Intervals", 
  sep=""), xlab="skeleton age", ylab="birthday", zlab="cti", zlim=zMinMax, 
  add=FALSE)
...
persp3d(x=outd$skelagePred, y=outd$birthdayPred, z=outd$z[[i]]$mean, 
  col=colors, main=paste(names(outd$z)[i],": ", (1-outd$alpha)*100,
    ...
    zlab="cti", zlim=zMinMax, add=FALSE)
